...
function process($hash, $size = DefaultSize, $default = '', $debugMode = false)
{
    ...
    // 临时参数
    $params = array('type' => null, 'path' => $filePath, 'code' => 302, 'uri' => AssetsPath . $subDir . $file . '.' . $size, 'useDef' => false);
    ...
    if ($tryCache) {
        ...
        if ($fileSize && $fileType && in_array($fileType, array('image/png', 'image/jpeg'))) {
            // 尝试进行缓存
    ...
    if ($params['code'] > 299 || $params['code'] < 200) {
        ...
        if ($params['code'] > 299 && $params['code'] < 400) {
            ...
            if (is_file($params['path'] . '.is.png')) {
                $params['type'] = 'image/png';
            } elseif (is_file($params['path'] . '.is.jpg')) {
